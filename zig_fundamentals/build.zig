...
pub fn build(b: *std.Build) void {
    ...
    const mod = b.addModule("zig_fundamentals", .{
    ...
    const exe = b.addExecutable(.{
        ...
        .root_module = b.createModule(.{
            ...
            // root module.
            .imports = &.{
                // Here "zig_fundamentals" is the name you will use in your source code to
                // import this module (e.g. `@import("zig_fundamentals")`). The name is
                // repeated because you are allowed to rename your imports, which
                // can be extremely useful in case of collisions (which can happen
                // importing modules from different packages).
                .{ .name = "zig_fundamentals", .module = mod },
            },
        }),
    });
    ...
    const pointer_mutability_exe = b.addExecutable(.{
        ...
        .root_module = b.createModule(.{
            .root_source_file = b.path("src/pointer_mutability.zig"),
            .target = target,
            .optimize = optimize,
        }),
    });
    b.installArtifact(pointer_mutability_exe);

    const run_pointer_mutability_step = b.step(
        "run-pointer-mutability",
        "Run the pointer mutability example",
    );
    const run_pointer_mutability_cmd = b.addRunArtifact(pointer_mutability_exe);
    ...
    const pointer_mutability_tests = b.addTest(.{
        .root_module = pointer_mutability_exe.root_module,
    });
